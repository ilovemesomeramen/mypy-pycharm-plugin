# mypy-pycharm (Multi-Module Fork)

> **WARNING: THIS FORK IS AI SLOP.** The multi-module support below was generated by an LLM (Claude, Anthropic) in collaboration with a human developer. It works, but read the code at your own discretion. Currently synced with upstream v2.2.2.

## What's new in this fork: Multi-Module Mypy Support

The upstream plugin only supports a single mypy configuration per project. If you attach multiple Python projects in one PyCharm window (File > Open > Attach), they all share one mypy executable, config file, and working directory — making the plugin unusable in multi-project setups.

This fork **automatically detects the correct mypy configuration per module** by resolving each module's Python SDK and content root. For cases where auto-detection isn't enough, there's also an explicit **per-module settings UI** under Settings > Tools > Mypy > Module Settings.

See [MULTI_MODULE_SUPPORT.md](MULTI_MODULE_SUPPORT.md) for full details on the changes.
...
The built plugin zip is at `build/distributions/`. No GitHub credentials are needed — the base library dependency is pulled via git include.
...
### Per-module configuration (this fork only)
In multi-module projects, the plugin auto-detects mypy from each module's Python SDK. For explicit control, go to **Settings > Tools > Mypy > Module Settings** where each module can be individually configured with its own mypy executable, config file, arguments, and working directory.
...
For multi-module resolution debugging, also add:
```
#works.szabope.plugins.mypy.services.MypyConfigurationResolver
```

## Acknowledgements
A huge thanks to [Peter Szabo](https://github.com/szabope) for the upstream plugin and to [Roberto Leinardi](https://github.com/leinardi) for the creation and maintenance of the original plugin.
